...
export async function detectLeoCli(
  ...
  return {
    available: false,
    mode: "unavailable",
    reason: LEO_UNAVAILABLE_REASON,
  };
}

export function getLeoCommand(detection: LeoCliDetection): { mode: LeoCommandMode; executable: string; args: string[] } {
  if (!detection.available) {
    return { mode: "unavailable", executable: "", args: [] };
  }
  if (detection.mode === "wsl") {
    return { mode: "wsl", executable: "wsl", args: ["bash", "-lc"] };
  }
  return { mode: "native", executable: "leo", args: [] };
}
...
function nativeLeoArgs(command: string) {
  if (!command.startsWith("leo ")) {
    return { executable: command, args: [] };
  }
  return {
    executable: "leo",
    args: command.slice("leo ".length).split(" ").filter(Boolean),
  };
}
...
export async function runLeoCommand(
  ...
  if (detection.mode === "wsl") {
    const wslProjectPath = windowsPathToWslPath(projectPath);
    const result = await runner("wsl", [
      "bash",
      "-lc",
      `cd ${shellQuote(wslProjectPath)} && ${command}`,
    ], { input: options.input });
  ...
  const native = nativeLeoArgs(command);
  const result = await runner(native.executable, native.args, {
    cwd: projectPath,
    input: options.input,
  });
  const sanitized = sanitizeLeoCommandResult(result);
