...
const walletMessages: Record<string, readonly [string, string]> = {
  ...
  "Shield 已注入，但适配器尚未就绪。请刷新页面后重新连接。": ["Shield 已注入，但适配器尚未就绪。请刷新页面后重新连接。", "Shield is injected, but its adapter is not ready. Refresh the page and reconnect."],
  ...
  "Shield 授权被取消或拒绝。请在扩展中移除本站授权后重新连接。": ["Shield 授权被取消或拒绝。请在扩展中移除本站授权后重新连接。", "Shield authorization was cancelled or rejected. Remove this site from the extension, then reconnect."],
  "Shield 当前网络与 Aleo Testnet 不一致。请切换到 Testnet 后重新连接。": ["Shield 当前网络与 Aleo Testnet 不一致。请切换到 Testnet 后重新连接。", "Shield is on a different network. Switch to Aleo Testnet, then reconnect."],
  ...
  "Shield 已安装但当前不可用。请解锁扩展并刷新页面后重试。": ["Shield 已安装但当前不可用。请解锁扩展并刷新页面后重试。", "Shield is installed but unavailable. Unlock it and refresh the page before trying again."],
  ...
  "Shield 钱包适配器无法初始化。": ["Shield 钱包适配器无法初始化。", "The Shield wallet adapter could not be initialized."],
  ...
  "钱包已接收 submit_claim；返回值目前只是 Wallet Request ID，不代表链上 Confirmed。": ["钱包已接收 submit_claim；当前返回值只是钱包请求编号，不代表链上已确认。", "The wallet accepted submit_claim. The current response is only a Wallet Request ID, not on-chain confirmation."],
  "钱包已返回 Public Transaction ID；仍需等待链上 Confirmed、Claim Receipt、Reporter 与 Nullifier Mapping 验证。": ["钱包已返回公开交易编号；仍需等待链上确认，并验证漏洞声明收据、报告人和防重复标识映射。", "The wallet returned a public Transaction ID. On-chain confirmation and Claim Receipt, Reporter, and Nullifier Mapping verification are still required."],
