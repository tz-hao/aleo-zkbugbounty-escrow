...
export function AleoCreateBountyForm() {
  ...
  return (
    <section className="surface-card rounded-lg p-5 sm:p-6" aria-labelledby="real-create-bounty-title">
      ...
      {!allowed ? (
        <div className="mt-5 rounded-lg border border-red-400/25 bg-red-500/10 p-3 text-sm text-red-100">
          当前演示角色不能打开 Project Owner 交易入口。Role Switcher 只控制 UI，链上权限最终由钱包签名者决定。
        </div>
      ) : null}
