...
type LookupState =
  | { kind: "loading" }
  | { kind: "found"; bounty: OnChainBountyState }
  | { kind: "missing"; message: string }
  | { kind: "unavailable"; message: string };

async function fetchPublicBounty(bountyId: string): Promise<LookupState> {
  try {
    const response = await fetch(`/api/aleo/bounties/${encodeURIComponent(bountyId)}`, {
      method: "GET",
    ...
    return { kind: "found", bounty: payload.bounty };
  } catch {
    ...
    <div className="grid gap-6">
      ...
      <section className="surface-card rounded-lg p-5 sm:p-6" aria-live="polite">
        {state.kind === "loading" ? (
          ...
          <BountyFacts bounty={state.bounty} />
        ) : (
...
function BountyFacts({ bounty }: { bounty: OnChainBountyState }) {
  return (
    <>
      ...
      <dl className="mt-4 grid gap-x-8 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <PublicFact label="Status" value={bounty.status} />
        ...
        <PublicFact label="Low Reward" value={`${bounty.rewards.low} microcredits`} />
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            className="focus-ring primary-action w-fit"
            href={`/submit-proof?bountyId=${encodeURIComponent(bounty.bountyId)}`}
          >
            提交 Wallet-signed Claim
