  ...
  return (
    <header className="site-navigation sticky top-0 z-40 px-3 pt-3">
      <div className="nav-glass mx-auto flex h-[3.75rem] max-w-7xl items-center gap-3 rounded-lg px-3 sm:px-4 lg:px-5">
        ...
        <span className="hidden items-center gap-2 rounded-md border border-cyan-300/20 bg-cyan-300/[0.06] px-2.5 py-1.5 text-[0.68rem] font-semibold text-cyan-100 xl:inline-flex">
            ...
            {links.map(({ href, label }) => {
              ...
              return (
                <Link className={`focus-ring relative inline-flex min-h-10 items-center gap-1.5 rounded-md px-3 text-xs font-medium transition xl:text-sm ${active ? "bg-white/[0.055] text-white" : "text-slate-400 hover:bg-white/[0.035] hover:text-slate-100"}`} href={href} key={href}>
                  {label}
                  {active ? <span className="absolute inset-x-3 -bottom-[0.65rem] h-px bg-gradient-to-r from-cyan-300/0 via-cyan-200 to-violet-300/0" aria-hidden="true" /> : null}
                </Link>
              );
            })}
          </div>
        ...
        <div className="nav-glass mx-auto mt-2 max-w-7xl rounded-lg p-3 shadow-2xl lg:hidden" id="mobile-navigation">
          ...
          <nav aria-label={text("移动端主导航", "Mobile navigation")} className="grid gap-1">
            {links.map(({ href, label, icon: Icon }) => {
              const active = pathname === href;
              return (
                <Link className={`focus-ring inline-flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-medium ${active ? "bg-gradient-to-r from-cyan-300/[0.11] to-violet-300/[0.08] text-cyan-50" : "text-slate-300 hover:bg-white/[0.05] hover:text-white"}`} href={href} key={href} onClick={() => setMobileOpen(false)}>
